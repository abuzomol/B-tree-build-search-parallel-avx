...
//B is number of items in a node
//Build tree horizontally where the last two layers are filled vertically
//size of items must equal B (B+1)^height
template <typename T, typename S>
void buildPerfectTreeHorizontalTwoLayers(std::vector<S> &items, std::vector<T> &tree,
                                         ll nodeIndex, ll start, ll end, ll height,
                                         ll B)
{
    ll powerH = binPow(B, height);
    ...
    //fill in the last level and level above it together
    for (ll j = 0; j < powerH * (B - 1); j++)
    {
        tree[powerH + j] = T(items[j]);
        if (j % (B - 1) == 0 && j % B != 0)
        {
            tree[powerI + j / (B - 1) - j / B] = T(items[j]);
        }
    }
    // fill in internal nodes
    for (ll i = height - 2; i >= 0; i--)
    {
        powerI /= B;
        powerHMinusI *= B;
        for (ll j = 0; j < powerI; j++)
...
//build tree horizontally with two layers at once!
template <typename T, typename S>
void buildTreeOptimalHorizontalTwoLayers(std::vector<S> &items, std::vector<T> &tree,
                                         ll nodeIndex, ll start, ll end, ll B)
{
    ll remainder = (end - start + 1) % (B - 1);
    // pad with infinities at the end
    ...
    {
        tree.reserve(power * B);
        buildPerfectTreeHorizontalTwoLayers(items, tree, nodeIndex, start, end, height, B);
    }
    else
    {
        ll x = ceil(B * z * 1.0 / (B - 1));
