                                      ...
                                      std::vector<T> &queries, ll B)
{
    ...
    {
        results.emplace_back(nodeSequentialSearch(tree, B, totalNodes, queries[i]));
    }
    return results;
}
                                       ...
                                       std::vector<T> &queries, ll B)
{
    ...
    for (ll i = 0; i < queries.size(); i++)
    {
        results.emplace_back(nodeBinarySearch(tree, B, totalNodes, queries[i]));
    }
    return results;
}
                                       ...
                                       std::vector<dtype> &queries, ll B)
{
    ...
    for (ll i = 0; i < queries.size(); i++)
    {   
        results.emplace_back(searchSimd(queries[i], B, totalNodes, tree, regBlocks));
    }
    return results;
}
